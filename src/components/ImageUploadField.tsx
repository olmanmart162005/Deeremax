...
export const ImageUploadField: React.FC<ImageUploadFieldProps> = ({
  ...
  return (
    <div className="dm-image-upload-field mb-4 w-full max-w-full min-w-0 box-border">
      {label ? (
        <div className="flex flex-wrap items-center justify-between gap-1 mb-2 w-full">
          <label className="text-xs font-extrabold text-slate-700 uppercase tracking-wider flex items-center gap-1.5 min-w-0">
            <Camera size={14} className="text-emerald-700 flex-shrink-0" />
            <span className="break-words">{label}</span>
          </label>
          <span className="text-[11px] text-slate-400 font-medium">
            {tipo === 'user' ? 'Avatar personal' : 'Logotipo'}
          </span>
        </div>
      ) : null}

      {/* Contenedor Principal de la Tarjeta Hero */}
      <div className="dm-photo-hero-card rounded-2xl border border-slate-200/90 bg-gradient-to-b from-slate-50/70 to-white p-3.5 sm:p-5 shadow-xs w-full max-w-full min-w-0 box-border overflow-hidden">
        
        {/* Zona Central de la Fotografía con Avatar Grande y Superposición */}
        <div className="flex flex-col items-center justify-center text-center pb-4 border-b border-slate-100 w-full min-w-0">
          <div
            className="relative group cursor-pointer select-none"
            onClick={() => fileInputRef.current?.click()}
          >
            <div className="p-1 rounded-full bg-white shadow-md ring-4 ring-emerald-600/10 transition-all duration-300 group-hover:ring-emerald-600/25">
              <Avatar
                src={value}
          ...
          <div className="mt-3 flex flex-col items-center w-full min-w-0 px-1">
            {value ? (
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-100/80 border border-emerald-300/80 text-[11px] font-bold text-emerald-800 max-w-full text-center break-words">
                <Sparkles size={12} className="text-emerald-700 flex-shrink-0" />
                <span>Fotografía personalizada activa</span>
              </div>
            ) : (
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-[11px] font-semibold text-slate-600 max-w-full text-center break-words">
                <span>Avatar corporativo con iniciales</span>
              </div>
            )}

            <p className="text-xs text-slate-500 mt-1.5 max-w-sm text-center leading-relaxed break-words">
              {value
                ? 'Puedes reemplazarla subiendo un archivo o pegando una nueva URL.'
            ...
            {value ? (
              <button
                type="button"
                className="mt-2 text-xs py-1 px-3 h-7 flex items-center justify-center gap-1.5 font-semibold text-rose-700 hover:text-rose-800 bg-rose-50 hover:bg-rose-100/80 border border-rose-200/80 rounded-lg transition-colors shadow-2xs"
        ...
        <div className="mt-3.5 space-y-3 w-full min-w-0">
          
          {/* OPCIÓN 1: Subir desde dispositivo */}
          <div className="dm-upload-option-card p-3 rounded-xl bg-white border border-slate-200/80 shadow-2xs w-full min-w-0 box-border">
            <div className="flex flex-wrap items-center justify-between gap-1 mb-2 w-full">
              <span className="text-xs font-bold text-slate-700 flex items-center gap-1.5 min-w-0">
                <span className="w-5 h-5 rounded-md bg-emerald-50 text-emerald-700 flex items-center justify-center text-[11px] font-black border border-emerald-200 flex-shrink-0">
                  1
                </span>
                <span className="break-words">Subir desde dispositivo</span>
              </span>
            ...
            <div
              className={`dm-dropzone-box flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 p-3 rounded-lg border-2 border-dashed transition-all duration-200 w-full min-w-0 box-border ${
                arrastrando
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0 text-left">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[10px] text-slate-400 leading-tight break-words">
                    {helpText || 'Ajuste y compresión automática'}
                  </p>
                </div>
              </div>
              ...
              <button
                type="button"
                className="btn-upload-file text-xs py-1.5 px-3.5 h-8.5 sm:h-8 flex items-center justify-center gap-1.5 font-bold text-white bg-emerald-700 hover:bg-emerald-800 rounded-lg shadow-xs transition-all w-full sm:w-auto flex-shrink-0"
                onClick={() => fileInputRef.current?.click()}
                disabled={disabled || estaCargando}
              >
                {procesandoArchivo ? (
          ...
          <div className="relative flex items-center justify-center py-0.5 w-full">
          ...
          <div className="dm-upload-option-card p-3 rounded-xl bg-white border border-slate-200/80 shadow-2xs w-full min-w-0 box-border">
            <div className="flex flex-wrap items-center justify-between gap-1 mb-2 w-full">
              <span className="text-xs font-bold text-slate-700 flex items-center gap-1.5 min-w-0">
                ...
                <span className="break-words">Pegar enlace de imagen (URL)</span>
              </span>
              <span className="text-[10px] text-slate-400 font-medium">
                Enlace web directo
              </span>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full min-w-0">
              <div className="field-shell flex-1 mb-0 relative w-full min-w-0 box-border">
                ...
                <input
                  ...
                  disabled={disabled || estaCargando}
                  className="text-xs pr-7 w-full min-w-0"
                />
                {urlInput ? (
                  <button
                    type="button"
              ...
              <button
                type="button"
                className={`text-xs py-2 sm:py-1.5 px-3.5 h-9 flex items-center justify-center gap-1.5 font-bold rounded-lg transition-all w-full sm:w-auto flex-shrink-0 ${
                  urlAplicadaExito
              ...
              >
                ...
                <span>{validandoUrl ? 'Verificando...' : urlAplicadaExito ? '¡Enlace aplicado!' : 'Aplicar URL'}</span>
              </button>
            </div>
            <p className="text-[10px] text-slate-400 mt-1.5 pl-0.5 leading-normal break-words">
              Pega cualquier URL pública directa y haz clic en <strong>&quot;Aplicar URL&quot;</strong> para previsualizar.
            </p>
          </div>
      ...
      {errorLocal ? (
        <div className="mt-2.5 p-2.5 sm:p-3 rounded-xl bg-rose-50 border border-rose-200 flex items-start gap-2.5 text-xs text-rose-800 font-medium animate-fadeIn shadow-2xs w-full min-w-0 box-border">
          <AlertCircle size={16} className="text-rose-600 flex-shrink-0 mt-0.5" />
          <div className="flex-1 leading-relaxed break-words min-w-0">{errorLocal}</div>
          <button
            type="button"
